...
public abstract class EffectImplicitIdTests
{
    ...
    public async Task SunshineActionTest(Task<IFunctionStore> storeTask)
    {
        ...
        await BusyWait.Until(() =>
            store.GetFunction(rAction.MapToStoredId(flowId.Instance)).SelectAsync(sf => sf?.Status == Status.Succeeded)
        );
        
        syncedCounter.Current.ShouldBe(1);
        var effectResults = await store.GetEffectResults(rAction.MapToStoredId(flowId.Instance));
        ...
        effectResults = await store.GetEffectResults(rAction.MapToStoredId(flowId.Instance));
        effectResults.Single(r => r.EffectId == 0.ToEffectId()).WorkStatus.ShouldBe(WorkStatus.Completed);
        syncedCounter.Current.ShouldBe(1);
    }
    ...
    public async Task ExceptionThrowingActionTest(Task<IFunctionStore> storeTask)
    {
        ...
        await BusyWait.Until(() =>
            store.GetFunction(rAction.MapToStoredId(flowId.Instance)).SelectAsync(sf => sf?.Status == Status.Failed)
        );
