...
public class CrashableFunctionStore : IFunctionStore
{
    ...
    )
    {
        ...
        var result = await _inner.PostponeFunction(storedId, postponeUntil, timestamp, expectedEpoch, complimentaryState);
    ...
    public Task<bool> SuspendFunction(StoredId storedId, long timestamp, int expectedEpoch,
        ComplimentaryState complimentaryState)
        => _crashed
            ? Task.FromException<bool>(new TimeoutException())
