...
public class TypeMapper(ITypeStore typeStore)
{
    ...
    private readonly ConcurrentDictionary<long, byte> _persisted = new();
    ...
    public long GetTypeId(Type type)
    {
        ...
        var existing = _serializedTypes.GetOrAdd(typeId, serializedType);
        if (!ReferenceEquals(existing, serializedType) && !existing.SequenceEqual(serializedType))
    ...
    public Task EnsurePersisted(IReadOnlyList<long> typeIds)
    {
        ...
        foreach (var typeId in typeIds)
            if (!_persisted.ContainsKey(typeId))
                (missing ??= []).Add(typeId);
    ...
    private async Task PersistMissing(List<long> missing)
    {
        ...
        if (unknown != null)
        {
            // An id without in-memory content was read from the store (an existing effect being re-persisted),
