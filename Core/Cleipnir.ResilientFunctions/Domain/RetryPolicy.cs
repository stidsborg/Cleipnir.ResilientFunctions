...
public class RetryPolicy(TimeSpan initialInterval, double backoffCoefficient, TimeSpan maximumInterval, int maximumAttempts, Func<Exception, bool> shouldRetry, TimeSpan suspendThreshold)
{
    ...
    public async Task<T> Invoke<T>(Func<Task<T>> work, Effect effect, UtcNow utcNow, FlowTimeouts flowTimeouts)
    {
        ...
        if (hasDelayUntil && delayUntil > utcNow())
            ...
            await effect.FlowExecutionState.WaitUntil(delayUntilId, delayUntil, utcNow);
        ...
        while (true)
        {
            ...
            catch (Exception e)
            {
                ...
                if (delay >= suspendThreshold)
                    //waits until the retry-delay expires - or parks forever if the flow suspends first
                    await effect.FlowExecutionState.WaitUntil(delayUntilId, delayUntil, utcNow);
                else
