...
public static class ElevatorPitchExample
{
    ...
    public static async Task ElevatorPitch(string connectionString)
    {
        ...
        var registration = rFunctions.Register( //making a function resilient is simply a matter of registering it
            ...
            inner: async Task<Return<string>>(string url) => await HttpClient.GetStringAsync(url) //this is the function you are making resilient!
        ...
        var result = await rFunc(functionInstanceId: "google", param: url); //invoking the function - btw you can F11-debug from here into your registered function
